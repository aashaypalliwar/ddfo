...
class NSGA2v1:
    ...
    def __init__(self, params):
        ...
        self.csv_writer = params["csv_writer"]
        self.track_gen = params["track_gen"]
        self.out = params["out"]
        
    def run(self):

        sdns = set()
        print("Genrating initial population")
        while len(sdns) < self.population_size:
            sdn = SDN.get_capacitated_random_sdn_placement(1, self.controllers_available, self.network, self.capacity, self.objectives)
            sat = True
            for constraint in self.constraints:
                sat = sat and constraint.check_if_satisfy(sdn)
#                print(constraint.check_if_satisfy(sdn), end=" ")
...
#            print()
        ...
        while counter != self.generations:
            ...
            data = {}
            for i in range(1, self.controllers_available + 1):
                data[i] = None
                
            for sdn in old_front:
                data[sdn.score["controller_count_score"]] = sdn.score["avg_sc_latency_score"]
    ...
    def mutate(self, sdn):
        ...
        new_controller_location = random.randint(0, sdn.switch_count-1)
        
        if len(controllers) == sdn.switch_count:
            return sdn
